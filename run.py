...
import os
print("CWD:", os.getcwd())
print("will create:", os.path.abspath("./checkpoints"))
os.makedirs("./checkpoints", exist_ok=True)
print("created? ", os.path.exists("./checkpoints"))
...
# modification Proceed + retrieval (memory bank)
parser.add_argument('--use_retrieval', action='store_true', default=False)
parser.add_argument('--bank_size', type=int, default=2048)
parser.add_argument('--k', type=int, default=8)
parser.add_argument('--tau', type=float, default=0.07)
parser.add_argument('--retrieval_alpha', type=float, default=0.8, help='Mixing weight: ref = alpha*recent + (1-alpha)*retrieved_scaled')
# + error gate threshold
# -----------------------
# Error-gated updates (Proceed)
# -----------------------
parser.add_argument('--use_err_gate', action='store_true',
                    help='Enable error-gated updates (skip optimizer.step when error is low)')
parser.add_argument('--adapt_top_p', type=float, default=1.0,
                    help='Update only the top p fraction of batches with highest loss (e.g., 0.2). 1.0 = always update')
parser.add_argument('--gate_window', type=int, default=256,
                    help='Window size W for dynamic threshold (quantile over recent losses)')
parser.add_argument('--warmup_steps', type=int, default=0,
                    help='Number of initial steps to always update before gating starts')
